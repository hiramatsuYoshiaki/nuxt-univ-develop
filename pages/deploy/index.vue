<template>
  <div class="container">
    ...
    <transition appear name="transitionScreen">
      <TransitionScreen v-if="page === '/deploy'" />
    </transition>
  </div>
</template>
<script>
import TransitionScreen from '~/components/transition/TransitionScreen.vue'
import ConDeploy from '~/components/content/deploy/ConDeploy.vue'
...
export default {
  ...
  data() {
    return {
      ...
      pageTitle: 'Deploy ',
      pageSubTitle: 'Step4',
      pageDiscription: '自動ディプロイ',
      pageDiscriptionDetail: 'GitHubからホストへ自動ディプロイする設定を作ります。'
    }
  },
  head() {
    return {
