<template>
  <div class="container">
    ...
    <transition appear name="transitionScreen">
      <TransitionScreen v-if="page === '/config'" />
    </transition>
  </div>
</template>
<script>
import TransitionScreen from '~/components/transition/TransitionScreen.vue'
import ConConfig from '~/components/content/config/ConConfig.vue'
...
export default {
  ...
  data() {
    return {
      ...
      pageTitle: 'Config ',
      pageSubTitle: 'Step2',
      pageDiscription: '初期設定',
      pageDiscriptionDetail: 'Nuxt.configの初期設定を行います。'
    }
  },
  head() {
    return {
